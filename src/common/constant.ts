...
function findRootDir(dir: string): string {
  if (existsSync(join(dir, 'lion.config.js'))) {
    return dir;
  }
  return dir;
...
export const LION_CONFIG_FILE = join(ROOT, 'lion.config.js');
...
export function getLionConfig() {
  delete require.cache[LION_CONFIG_FILE];

  try {
    return require(LION_CONFIG_FILE);
  } catch (err) {
    return {};
  }
}
...
function getSrcDir() {
  ...
  const srcDir = get(lionConfig, 'build.srcDir');
